...
public class SilantroAerofoil : MonoBehaviour {
	...
	[HideInInspector]public List<float> flapAngles;
	...
	public enum TrimState{Absent,Available}[HideInInspector]public TrimState trimState = TrimState.Absent;float baseSpeed;
	...
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	//CONTROL INPUTS
	[HideInInspector]public float pitchInput,rollInput,yawInput,baseInput = 0f;

	//MAIN CONTROL SURFACE ACTUATOR SETTINGS
	[HideInInspector] public float actuationSpeed = 60f;
	[HideInInspector] public float maximumTorque = 500f;
	[HideInInspector] public float actuatorDeflection;
	[HideInInspector] public AnimationCurve controlEfficiencyCurve;
	[HideInInspector] public float C100, C50, C10,C0,CA;

	//FLAP ACTUATOR SETTINGS
	[HideInInspector] public float flapActuationSpeed = 50f;
	[HideInInspector] public float maximumFlapActuatorTorque = 600f;
	[HideInInspector] public float flapActuatorDeflection;
	[HideInInspector] public AnimationCurve flapEfficiencyCurve;
	[HideInInspector] public float F100, F50, F10, F0,FA;

	//SPOILER ACTUATOR SETTINGS
	[HideInInspector] public float spoilerActuationSpeed = 50f;
	[HideInInspector] public float maximumSpoilerActuatorTorque = 600f;
	[HideInInspector] public float spoilerActuatorDeflection;
	[HideInInspector] public AnimationCurve spoilerEfficiencyCurve;
	[HideInInspector] public float S100, S50, S10, S0, SA;

	[HideInInspector] public List<String> toolbarStrings;
	...
	public void InitializeFoil()
	{
		...
		if(allOk){
			...
			C100 = EstimateEfficiencySpeed(1, controlArea,maximumTorque);C50 = EstimateEfficiencySpeed(0.5f, controlArea, maximumTorque);C10 = EstimateEfficiencySpeed(0.1f, controlArea, maximumTorque); C0 = EstimateEfficiencySpeed(0.01f, controlArea, maximumTorque);
			if (canUseFlap) {
				F100 = EstimateEfficiencySpeed(1, flapArea, maximumFlapActuatorTorque);
				F50 = EstimateEfficiencySpeed(0.5f, flapArea, maximumFlapActuatorTorque);
				F10 = EstimateEfficiencySpeed(0.1f, flapArea, maximumFlapActuatorTorque);
				F0 = EstimateEfficiencySpeed(0.01f, flapArea, maximumFlapActuatorTorque);
			}
			if(canUseSpoilers)
			{
				S100 = EstimateEfficiencySpeed(1, spoilerArea, maximumSpoilerActuatorTorque);
				S50 = EstimateEfficiencySpeed(0.5f, spoilerArea, maximumSpoilerActuatorTorque); 
				S10 = EstimateEfficiencySpeed(0.1f, spoilerArea, maximumSpoilerActuatorTorque); 
				S0 = EstimateEfficiencySpeed(0.01f, spoilerArea, maximumSpoilerActuatorTorque);
			}

			controlEfficiencyCurve = new AnimationCurve();flapEfficiencyCurve = new AnimationCurve();spoilerEfficiencyCurve = new AnimationCurve();
			//PLOT CONTROL SURFACE EFFICIENCY
			//BASE CONTROL
			if (availableControls != AvailableControls.SecondaryOnly) { for (float i = C100; i < 1000f; i += 50f) { float efficiency = EstimateControlEfficiency(i, controlArea, maximumTorque); 
			controlEfficiencyCurve.AddKey(new Keyframe(i*1.944f, efficiency*100f)); } }
			//FLAP
			if (aerofoilType == AerofoilType.Wing && canUseFlap && availableControls != AvailableControls.PrimaryOnly) {
			for (float i = F100; i < 1000f; i += 50f){float flapEfficiency = EstimateControlEfficiency(i, flapArea,maximumFlapActuatorTorque);flapEfficiencyCurve.AddKey(new Keyframe(i * 1.944f, flapEfficiency*100f));}}
			//SPOILER
			if (aerofoilType == AerofoilType.Wing && canUseSpoilers && availableControls != AvailableControls.PrimaryOnly)
			{
				for (float i = S100; i < 1000f; i += 50f) { float spoilerEfficiency = EstimateControlEfficiency(i, spoilerArea, maximumSpoilerActuatorTorque); spoilerEfficiencyCurve.AddKey(new Keyframe(i * 1.944f, spoilerEfficiency * 100f)); }
			}
		}
	}
	...
	void Update()
	{
		...
		baseSpeed = connectedAircraft.velocity.magnitude;
		...
		if (controlDeflectionType == DeflectionType.Asymmetric) {
		if (transform.localScale.x < 0) { actuatorDeflection = baseInput > 0f ? baseInput * negativeLimit : baseInput * positveLimit; }
		if (transform.localScale.x > 0) { actuatorDeflection = baseInput > 0f ? baseInput * positveLimit : baseInput * negativeLimit; }}

		//SYMMETRIC DEFLECTION
		if (controlDeflectionType == DeflectionType.Symmetric) { actuatorDeflection = baseInput > 0f ? baseInput * positveLimit : baseInput * positveLimit; }

		//CONTROL DEFLECTION BASED ON APPLIED FORCE
		if (baseSpeed > 1)
		{
			float surfaceEffectiveness = EstimateControlEfficiency(baseSpeed, controlArea,maximumTorque);CA = surfaceEffectiveness;
			if (!float.IsNaN(surfaceEffectiveness)) { actuatorDeflection *= Mathf.Clamp01(surfaceEffectiveness); }
		}}}
		...
		if (aerofoilType == AerofoilType.Wing && canUseFlap && availableControls != AvailableControls.PrimaryOnly) {
			...
			if (flapType == FlapType.Flaperon) {
				...
				if (transform.localScale.x < 0) { baseRoll = rollInput > 0f ? rollInput * negativeLimit : rollInput * positveLimit; }
				if (transform.localScale.x > 0) { baseRoll = rollInput > 0f ? rollInput * positveLimit : rollInput * negativeLimit; }
				if (flaperonPosition == FlaperonPosition.Right) {baseFlap = -flapAngle;} else {baseFlap = flapAngle;}
				//MIX
				flapActuatorDeflection = ((baseFlap*2f) + (baseRoll*2f))/2f;
				if (transform.localScale.x > 0) { flapActuatorDeflection = Mathf.Clamp(flapActuatorDeflection, -negativeLimit, positveLimit); }
				if (transform.localScale.x < 0) { flapActuatorDeflection = Mathf.Clamp(flapActuatorDeflection, -positveLimit, negativeLimit); }
			} 
			else {
				if (flapType == FlapType.Flaperon) {if (flaperonPosition == FlaperonPosition.Right) { flapActuatorDeflection = -flapAngle;} else { flapActuatorDeflection = flapAngle;}} 
				else {
				if (horizontalPosition == HorizontalPosition.Right) { flapActuatorDeflection = -flapAngle;}if (horizontalPosition == HorizontalPosition.Left) { flapActuatorDeflection = flapAngle;}
				if (horizontalPosition == HorizontalPosition.Center) {if (centerSide == CenterSide.Right) { flapActuatorDeflection = -flapAngle;}if (centerSide == CenterSide.Left) { flapActuatorDeflection = flapAngle;}}}
			}

			//CONTROL DEFLECTION
			if(baseSpeed > 1)
			{
				float flapEffectiveness = EstimateControlEfficiency(baseSpeed, flapArea,maximumFlapActuatorTorque); FA = flapEffectiveness;
	...
	void FixedUpdate () {
		...
		if (isControllable) {
			...
			//MOVE ACTUATOR
			//1.CANARD
			if (aerofoilType == AerofoilType.Canard) { controlDeflection = Mathf.MoveTowards(controlDeflection, -actuatorDeflection, actuationSpeed * Time.fixedDeltaTime); }
			//2. NORMAL
			else{ controlDeflection = Mathf.MoveTowards(controlDeflection, actuatorDeflection, actuationSpeed * Time.fixedDeltaTime);}

			//MOVE FLAP
			flapDeflection = Mathf.MoveTowards(flapDeflection, flapActuatorDeflection, flapActuationSpeed * Time.fixedDeltaTime);
	...
	List<float> setionAOA,sectionLift;
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	private void BuildFoilForces()
	{
		...
		setionAOA = new List<float>();sectionLift = new List<float>();
		...
		for (int p = 0; p < foilSubdivisions; p++) {
			...
			if(controlState == ControType.Controllable){
				...
				if (canUseFlap && aerofoilType == AerofoilType.Wing && flapSections [p] == true) {
				//ADD EXTENSION LINE
				...
				if (surfaceType != SurfaceType.Inactive && availableControls != AvailableControls.SecondaryOnly && controlSections [p] == true) {
				//ADD EXTENSION LINE
			...
			if (angleOfAttack > -180f || angleOfAttack < 180) {
				...
				//SLAT CORRECTION
				if (controlState == ControType.Controllable && canUseSlat && slatSections [p] == true) {effectiveLiftCoefficient += slatLiftFactor;}
				...
				if (controlState == ControType.Controllable && spoilerSections [p] == true && canUseSpoilers && aerofoilType == AerofoilType.Wing) {
				...
				TotalLift += panelLift;
				sectionLift.Add(panelLift);

				//CONSIDER SLAT EFFECTS
				if (controlState == ControType.Controllable && slatSections [p] == true ) {
				...
				float effectiveMomentCoefficient = EstimateEffectiveValue (rootMomentCoefficient, tipMomentCoefficient, panelDistance, 0);
				if (controlState == ControType.Controllable && canUseSlat && slatSections [p] == true) {effectiveMomentCoefficient += slatMomentFactor;}
				...
				float effectiveDragCoefficient = EstimateEffectiveValue (rootDragCoefficient, tipDragCoefficient, panelDistance, 0);
				if (controlState == ControType.Controllable && canUseSlat && slatSections [p] == true) {effectiveDragCoefficient += slatDragFactor;}
				...
				if (controlState == ControType.Controllable && spoilerSections [p] == true) {panelDrag += panelSpoilerDrag;}panelDragForce = airflow;panelDragForce.Normalize ();
	...
	void OnDrawGizmos()
	{
		...
		C100 = EstimateEfficiencySpeed(1, controlArea, maximumTorque);
		C50 = EstimateEfficiencySpeed(0.5f, controlArea, maximumTorque);
		C10 = EstimateEfficiencySpeed(0.1f, controlArea, maximumTorque); 
		C0 = EstimateEfficiencySpeed(0.01f, controlArea, maximumTorque);
		...
		//PLOT CONTROL SURFACE EFFICIENCY
		//BASE CONTROL
		for (float i = C100; i < 1000f; i += 50f) { float efficiency = EstimateControlEfficiency(i, controlArea, maximumTorque); controlEfficiencyCurve.AddKey(new Keyframe(i * 1.944f, efficiency * 100f)); }
		...
		if (aerofoilType == AerofoilType.Wing && canUseSpoilers && availableControls != AvailableControls.PrimaryOnly)
		{
			for (float i = S100; i < 1000f; i += 50f) { float spoilerEfficiency = EstimateControlEfficiency(i, spoilerArea, maximumSpoilerActuatorTorque); 
			spoilerEfficiencyCurve.AddKey(new Keyframe(i * 1.944f, spoilerEfficiency * 100f)); }
		}
	...
	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	public float EstimateControlEfficiency(float inputSpeed, float inputArea, float inputTorque)
	{
		float efficiency = Mathf.Asin(inputTorque / (inputSpeed * inputSpeed * inputArea)) * Mathf.Rad2Deg;
		return efficiency;
	}

	// ----------------------------------------------------------------------------------------------------------------------------------------------------------
	public float EstimateEfficiencySpeed(float inputEfficiency, float inputArea, float inputTorque)
	{
...
public class AerofoilEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		GUI.color = backgroundColor;
		GUILayout.Space (3f);
		aerofoil.taperPercentage = EditorGUILayout.Slider ("Taper Percentage", aerofoil.taperPercentage, 0f, 95f);
		GUILayout.Space (5f);
		...
		if (aerofoil.controlState == SilantroAerofoil.ControType.Controllable) {
			GUILayout.Space(5f);
			if (aerofoil.aerofoilType == SilantroAerofoil.AerofoilType.Wing){aerofoil.availableControls = (SilantroAerofoil.AvailableControls)EditorGUILayout.EnumPopup(" ", aerofoil.availableControls);}
			...
			if (aerofoil.aerofoilType == SilantroAerofoil.AerofoilType.Wing && aerofoil.availableControls != SilantroAerofoil.AvailableControls.PrimaryOnly) {
				...
				GUILayout.Space(2f);
				GUILayout.BeginVertical (GUILayout.Width(110));
				aerofoil.canUseFlap = EditorGUILayout.Toggle ("Flaps", aerofoil.canUseFlap); GUILayout.Space(2f);
				aerofoil.canUseSlat = EditorGUILayout.Toggle ("Slats", aerofoil.canUseSlat); GUILayout.Space(2f);
			...
			if (aerofoil.availableControls == SilantroAerofoil.AvailableControls.PrimaryOnly)
			{
				...
				if (!aerofoil.toolbarStrings.Contains("Primary")) { aerofoil.toolbarStrings.Add("Primary"); }switch (aerofoil.toolbarTab)
				{
			...
			if (aerofoil.aerofoilType == SilantroAerofoil.AerofoilType.Wing && aerofoil.availableControls == SilantroAerofoil.AvailableControls.PrimaryPlusSecondary)
			{
				...
				if (aerofoil.canUseFlap && !aerofoil.toolbarStrings.Contains("Flap")) { aerofoil.toolbarStrings.Add("Flap"); }
				if (aerofoil.canUseSlat && !aerofoil.toolbarStrings.Contains("Slat")) { aerofoil.toolbarStrings.Add("Slat"); }
				if (aerofoil.canUseSpoilers && !aerofoil.toolbarStrings.Contains("Spoiler")) { aerofoil.toolbarStrings.Add("Spoiler"); }
				GUILayout.Space(5f);
			...
			switch (aerofoil.currentTab)
			{
				...
				case "Primary":
					if (aerofoil.availableControls != SilantroAerofoil.AvailableControls.SecondaryOnly)
					{
						...
						if (aerofoil.surfaceType != SilantroAerofoil.SurfaceType.Inactive)
						{
							...
							GUILayout.Space(5f);
							GUI.color = Color.white;
							aerofoil.actuationSpeed = EditorGUILayout.FloatField("Actuation Speed (°/s)", aerofoil.actuationSpeed);
							GUILayout.Space(3f);
							aerofoil.maximumTorque = EditorGUILayout.FloatField("Torque Limit (Nm)", aerofoil.maximumTorque);
							GUILayout.Space(3f);
							float currentEff = (aerofoil.CA * 100f); currentEff = Mathf.Clamp(currentEff, 0, 100f);
							...
							GUI.color = backgroundColor;
							GUILayout.Space(3f);
							float machLock = aerofoil.C10 / 343f;
							EditorGUILayout.LabelField("Lock-Up Speed", (aerofoil.C10 * 1.944f).ToString("0.0") + " knots" + " (M" + machLock.ToString("0.00") + ")");
							GUILayout.Space(3f);
							float zeroPoint = aerofoil.C0 / 343f;
							EditorGUILayout.LabelField("Zero Authority Speed", (aerofoil.C0 * 1.944f).ToString("0.0") + " knots" + " (M" + zeroPoint.ToString("0.00") + ")");
				...
				case "Flap":
					if (aerofoil.availableControls != SilantroAerofoil.AvailableControls.PrimaryOnly && aerofoil.canUseFlap)
					{
						...
						GUILayout.Space(5f);
						GUI.color = backgroundColor;
						aerofoil.flapRootChord = EditorGUILayout.Slider("Flap Root Chord :",aerofoil.flapRootChord, 0f, 100f);
						GUILayout.Space(2f);
						aerofoil.flapTipChord = EditorGUILayout.Slider("Flap Tip Chord :", aerofoil.flapTipChord, 0f, 100f);
						...
						GUILayout.Space(5f);
						GUI.color = Color.white;
						aerofoil.flapActuationSpeed = EditorGUILayout.FloatField("Actuation Speed (°/s)", aerofoil.flapActuationSpeed);
						GUILayout.Space(3f);
						aerofoil.maximumFlapActuatorTorque = EditorGUILayout.FloatField("Torque Limit (Nm)", aerofoil.maximumFlapActuatorTorque);
						GUILayout.Space(3f);
						float currentEff = (aerofoil.FA * 100f); currentEff = Mathf.Clamp(currentEff, 0, 100f);
						EditorGUILayout.LabelField("Current Efficiency", currentEff.ToString("0.0") + " %");
						GUILayout.Space(3f);
						...
						GUI.color = backgroundColor;
						GUILayout.Space(3f);
						float machLock = aerofoil.F10 / 343f;
						EditorGUILayout.LabelField("Lock-Up Speed", (aerofoil.F10 * 1.944f).ToString("0.0") + " knots" + " (M" + machLock.ToString("0.00") + ")");
						GUILayout.Space(3f);
						float zeroPoint = aerofoil.F0 / 343f;
						EditorGUILayout.LabelField("Zero Authority Speed", (aerofoil.F0 * 1.944f).ToString("0.0") + " knots" + " (M" + zeroPoint.ToString("0.00") + ")");
				...
				case "Slat":
					if (aerofoil.availableControls != SilantroAerofoil.AvailableControls.PrimaryOnly && aerofoil.canUseSlat)
					{
						...
						GUILayout.Space(5f);
						GUI.color = backgroundColor;
						aerofoil.slatRootChord = EditorGUILayout.Slider("Slat Root Chord :", aerofoil.slatRootChord, 0f, 100f);
						GUILayout.Space(2f);
						aerofoil.slatTipChord = EditorGUILayout.Slider("Slat Tip Chord :", aerofoil.slatTipChord, 0f, 100f);
						...
						if (aerofoil.slatMovement == SilantroAerofoil.SlatMovement.Deflection)
						{
							...
							GUILayout.Space(3f);
							aerofoil.slatActuationSpeed = EditorGUILayout.Slider("Actuation Speed",aerofoil.slatActuationSpeed, 10, 50);
							GUILayout.Space(3f);
						...
						if (aerofoil.slatMovement == SilantroAerofoil.SlatMovement.Extension)
						{
							...
							GUILayout.Space(3f);
							aerofoil.slatActuationSpeed = EditorGUILayout.Slider("Actuation Speed", aerofoil.slatActuationSpeed, 1, 10);
							GUILayout.Space(3f);
							aerofoil.slatActuationTime = EditorGUILayout.FloatField("Actuation Time", aerofoil.slatActuationTime);
							GUILayout.Space(3f);
				...
				case "Spoiler":
					if (aerofoil.availableControls != SilantroAerofoil.AvailableControls.PrimaryOnly && aerofoil.canUseSpoilers)
					{
						...
						GUILayout.Space(5f);
						GUI.color = backgroundColor;
						aerofoil.spoilerChordFactor = EditorGUILayout.Slider("Spoiler Chord (xst/c):", aerofoil.spoilerChordFactor, 0f, 60f);
						GUILayout.Space(2f);
						aerofoil.spoilerHinge = EditorGUILayout.Slider("Spoiler Hinge:", aerofoil.spoilerHinge, 0f, 100f);
						...
						GUILayout.Space(5f);
						GUI.color = Color.white;
						aerofoil.spoilerActuationSpeed = EditorGUILayout.FloatField("Actuation Speed (°/s)", aerofoil.spoilerActuationSpeed);
						GUILayout.Space(3f);
						aerofoil.maximumSpoilerActuatorTorque = EditorGUILayout.FloatField("Torque Limit (Nm)", aerofoil.maximumSpoilerActuatorTorque);
						GUILayout.Space(3f);
						float currentEff = (aerofoil.SA * 100f); currentEff = Mathf.Clamp(currentEff, 0, 100f);
						EditorGUILayout.LabelField("Current Efficiency", currentEff.ToString("0.0") + " %");
						GUILayout.Space(3f);
						...
						GUI.color = backgroundColor;
						GUILayout.Space(3f);
						float machLock = aerofoil.S10 / 343f;
						EditorGUILayout.LabelField("Lock-Up Speed", (aerofoil.S10 * 1.944f).ToString("0.0") + " knots" + " (M" + machLock.ToString("0.00") + ")");
						GUILayout.Space(3f);
						float zeroPoint = aerofoil.S0 / 343f;
						EditorGUILayout.LabelField("Zero Authority Speed", (aerofoil.S0 * 1.944f).ToString("0.0") + " knots" + " (M" + zeroPoint.ToString("0.00") + ")");
