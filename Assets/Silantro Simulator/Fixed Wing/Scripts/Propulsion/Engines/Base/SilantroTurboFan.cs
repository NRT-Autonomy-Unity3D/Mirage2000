...
public class SilantroTurboFan : MonoBehaviour {
	...
	//[HideInInspector]
	public float actualCED,actualFED;
	...
	void OnDrawGizmos()
	{
		...
		if (engineType == EngineType.Unmixed) {
			actualCED = EngineDiameter*(coreExhaustDiameterPercentage/100f);actualFED = EngineDiameter * (fanExhaustDiameterPercentage / 100f);
		...
		if(IntakePoint != null && IntakePoint!=null){Handles.DrawWireDisc (IntakePoint.transform.position, IntakePoint.transform.forward, (actualIntakeDiameter / 2f));}
