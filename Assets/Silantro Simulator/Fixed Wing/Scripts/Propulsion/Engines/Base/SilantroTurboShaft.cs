...
public class SilantroTurboShaft : MonoBehaviour {
	...
	float drawDiameter;
	//EFFICIENCIES
	[HideInInspector]public float pcc = 6f,nd = 92f,nc = 95f,nb = 98f,nt = 97f,nab = 92f,nm = 96f,ng = 95f,nft = 90f;
	public float Wc,alpha,Wt,Wshaft,Pshaft,Tj,Pt,brakePower,Hc,mach;
	...
	private void EngineCalculation()
	{
		...
		Pa = EMU.ambientPressure;
		Ta = 273.5f + EMU.ambientTemperature;
		...
		float p5_p4 = (T4 - T5) / ((nt / 100) * T4);
		P5 = P4*Mathf.Pow((1-p5_p4),(Y5/(Y5-1)));
		float p6_p4 = Mathf.Pow ((Pa / P4), ((Y5 - 1) / Y5));
		Hc = Cp5 * T4 * (1 - p6_p4);

		//NOZZLE========================================================================
		P6 = Pa;
		float t5_t4 = 1 - Mathf.Pow ((Pa / P5), ((Y5 - 1) / Y5));
		T6 = T5 * (1 - ((nft / 100) * t5_t4));
		...
		Wt = (nt / 100) * alpha * Hc;
		Wshaft = (1 + f) * Cp6 * 1000 * (T5 - T6);
		Pshaft = Ma * Wshaft*(nm/100);
		exhaustTemperature = T6 - 273.15f;
		//
		Pt = ((ng/100)*Pshaft);
		Pt /= 1000f;
		brakePower = (Pt / 0.7457f)*coreFactor;if(brakePower < 0){brakePower = 0;}

		TSFC = ((Mf * 3600f) / (brakePower));actualConsumptionrate = Mf*coreFactor;
	}
}
...
public class PlumSilantroTurboShaftEditor: Editor
{
	...
	SilantroTurboShaft jet;SerializedObject engineObject;
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space(5f);
		jet.intakeType = (SilantroTurboShaft.IntakeShape)EditorGUILayout.EnumPopup ("Intake Type", jet.intakeType);
		GUILayout.Space(5f);
		...
		jet.fuelType = (SilantroTurboShaft.FuelType)EditorGUILayout.EnumPopup ("Fuel Type", jet.fuelType);
		GUI.color = backgroundColor;
		if (jet.fuelType == SilantroTurboShaft.FuelType.JetB){jet.combustionEnergy = 42.8f;}
		else if (jet.fuelType == SilantroTurboShaft.FuelType.JetA1) {jet.combustionEnergy = 43.5f;}
		else if (jet.fuelType == SilantroTurboShaft.FuelType.JP6) {jet.combustionEnergy = 49.6f;} 
		else if (jet.fuelType == SilantroTurboShaft.FuelType.JP8) {jet.combustionEnergy = 43.28f;}
		jet.combustionEnergy *= 1000f;
		GUILayout.Space(6f);
		EditorGUILayout.LabelField ("Q ", jet.combustionEnergy.ToString ("0.00") + " KJ");
		GUILayout.Space(3f);
		EditorGUILayout.LabelField ("TSFC ", jet.TSFC.ToString ("0.00") + " lb/hp.hr");
		...
		GUILayout.Space(3f);
		jet.nt = EditorGUILayout.Slider ("Gas Turbine",jet.nt,90,95);
		GUILayout.Space(3f);
		jet.nft = EditorGUILayout.Slider ("Power Turbine",jet.nft,90,98);
		GUILayout.Space(3f);
		jet.nft = EditorGUILayout.Slider ("Power Turbine",jet.nft,90,99);
		GUILayout.Space(3f);
		jet.nm = EditorGUILayout.Slider ("Turbine Mechanical",jet.nm,90,99);
		GUILayout.Space(3f);
		...
		GUILayout.Space(3f);
		jet.rotationAxis = (SilantroTurboShaft.RotationAxis)EditorGUILayout.EnumPopup("Rotation Axis",jet.rotationAxis);
		GUILayout.Space(3f);
		jet.rotationDirection = (SilantroTurboShaft.RotationDirection)EditorGUILayout.EnumPopup("Rotation Direction",jet.rotationDirection);
		//
		GUILayout.Space(5f);
		...
		GUI.color = backgroundColor;
		GUILayout.Space(3f);
		jet.soundState = (SilantroTurboShaft.SoundState)EditorGUILayout.EnumPopup("Cabin Sounds", jet.soundState);
		GUILayout.Space(5f);
		if (jet.soundState == SilantroTurboShaft.SoundState.Absent) {
