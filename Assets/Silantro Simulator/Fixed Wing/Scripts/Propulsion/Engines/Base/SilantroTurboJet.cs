...
public class SilantroTurboJet : MonoBehaviour {
	...
	//-------------------------------------------CASES
	[HideInInspector]public float Pa,P1,P2,P3,P4,P5,P6,P7,Pc;
	[HideInInspector]public float Ta,T1,T2,T3,T4,T5,T6,T7;
	[HideInInspector]public float Ya, Y1, Y2, Y3, Y4, Y5, Y6, Y7;
	[HideInInspector]public float Cpa,Cp1,Cp2,Cp3,Cp4,Cp5,Cp6,Cp7;
	...
	[HideInInspector]public float pressureRatio = 10f, combustionEnergy;[HideInInspector]public float f,fab,Ae;
	[HideInInspector]public float Aeb;float drawDiameter;
	//EFFICIENCIES
	[HideInInspector]public float pcc = 6f,pcab = 3f,nd = 92f,nc = 95f,nb = 98f,nt = 97f,nab = 92f,nn = 96f;
	...
	{
		if(reheatSystem == ReheatSystem.Afterburning && corePower > 0.5f && FuelInput > 0.5f) {afterburnerOperative = !afterburnerOperative;}
	}
	...
	void OnDrawGizmos()
	{
		...
		if (!afterburnerOperative) {
			drawDiameter = ExhaustDiameter;
		} else {
			drawDiameter = ExhaustDialation;
		}
		//
	...
	private void EngineCalculation()
	{
		...
		f = (F1 / F2)*FuelInput;fab = 0;

		//TURBINE========================================================================
		Y5 =  adiabaticFactor.Evaluate (T5);Cp5 = pressureFactor.Evaluate(T5)*1000f;
		T5 = T4 - ((Cp3 * (T3 - T2)) / (Cp5 * (1 + f)));
		float p5_4 = 1 - ((T4 - T5) / ((nt / 100) * T4));
		P5 = P4 * (Mathf.Pow (p5_4, (Y5 / (Y5 - 1))));

		float AAB = (1 / (nn / 100)) * ((Y5 - 1) / (Y5 + 1));
		float AAC = 1 - AAB;
		float superPressure = 1 / (Mathf.Pow (AAC, (Y5 / (Y5 - 1))));

		//NOZZLE========================================================================
		P6 = P5;Pc = P6 / 1.893f;T6 = T5;
		...
		if (Pc >= Pa) {
			...
			P7 = Pa;
			T7Factor1 = (2 * Cp6 * T6);T7factor2 = (1 - ((Mathf.Pow ((Pa / P6), (Y6 - 1) / Y6))));
			exhaustVelocity = Mathf.Sqrt (T7Factor1 * T7factor2);
		}
		T7 = T6 / ((Y6 + 1) / 2f);

		//EXHAUST VELOCITY
		exhaustVelocity = Mathf.Sqrt (1.4f * 287f * T7);
		...
		if (afterburnerOperative) {
			P6 = (1-(pcab/100))*P5;
			T6 = MaximumTemperature;
			fab = (((Cp5/1000) * (T6 - T5)) / (((nab / 100) * combustionEnergy) - ((Cp5/1000) * T6)));
			Y6 = adiabaticFactor.Evaluate (T6);Cp6 = pressureFactor.Evaluate(T6);
			...
			if (Pc >= Pa) {
				...
				P7 = Pa;
				float T7Factor1 = (2 * Cp6 * T6);
				float T7factor2 = (1 - ((Mathf.Pow ((Pa / P6), (Y6 - 1) / Y6))));
				exhaustVelocity = Mathf.Sqrt (T7Factor1 * T7factor2);
			}
			Aeb = (287f*T7*Ma*(1+f+fab))/(P7*1000*exhaustVelocity);
			if (Aeb > 0) {ExhaustDialation = Mathf.Sqrt ((Aeb * 4f) / (3.142f));}
			Mf = Ma * (f + fab);
		}

		exhaustTemperature = T7 - 273.15f;
		Y7 = adiabaticFactor.Evaluate (T7);
		//EXHAUST VELOCITY
		Cp7 = pressureFactor.Evaluate(T7);
		exhaustVelocity = Mathf.Sqrt (1.4f * 287f * T7);exhaustVelocity *= coreFactor;exhaustTemperature *= coreFactor;

		if (!afterburnerOperative) {
			//THRUST
			CoreThrust = (((1 + f) * exhaustVelocity) - (speed)) * Ma;
			PressureThrust = (Ae * ((P7 - Pa) * 1000));
			EngineThrust = (CoreThrust + PressureThrust)*coreFactor;
		} else {
			CoreThrust = (((1 + f+fab) * exhaustVelocity) - (speed)) * Ma;
			PressureThrust = (Aeb * ((P7 - Pa) * 1000));
			EngineThrust = (CoreThrust + PressureThrust)*coreFactor;
		}
...
public class SilantroTurboJetEditor: Editor
{
	...
	SilantroTurboJet jet;SerializedObject engineObject;
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space(5f);
		jet.intakeType = (SilantroTurboJet.IntakeShape)EditorGUILayout.EnumPopup ("Intake Type", jet.intakeType);
		GUILayout.Space(5f);
		...
		GUI.color = backgroundColor;
		if (jet.fuelType == SilantroTurboJet.FuelType.JetB){jet.combustionEnergy = 42.8f;}
		else if (jet.fuelType == SilantroTurboJet.FuelType.JetA1) {jet.combustionEnergy = 43.5f;}
		else if (jet.fuelType == SilantroTurboJet.FuelType.JP6) {jet.combustionEnergy = 49.6f;} 
		else if (jet.fuelType == SilantroTurboJet.FuelType.JP8) {jet.combustionEnergy = 43.28f;}
