...
public class SilantroGun : MonoBehaviour {
	...
	//SOUND
	[HideInInspector]public AudioClip fireLoopSound;
	[HideInInspector]public AudioClip fireEndSound;
	[HideInInspector]public float soundVolume = 0.75f;
	[HideInInspector]public AudioSource gunLoopSource,gunEndSource;
	float bulletMass;
	...
	public void InitializeGun()
	{
		...
		gunLoopSource = speaker.AddComponent<AudioSource> ();
		//SET PROPS
		if(fireLoopSound != null)
		{
			gunLoopSource.loop = true;gunLoopSource.clip = fireLoopSound;
			gunLoopSource.dopplerLevel = 0f;
			gunLoopSource.spatialBlend = 1f;
			gunLoopSource.rolloffMode = AudioRolloffMode.Custom;
			gunLoopSource.maxDistance = 150f;
			gunLoopSource.volume = soundVolume;
		}
		if(fireEndSound != null)
		{
			gunEndSource = speaker.AddComponent<AudioSource>();
			//SET
			gunEndSource.loop = false;
			gunEndSource.dopplerLevel = 0f;
			gunEndSource.spatialBlend = 1f;
			gunEndSource.rolloffMode = AudioRolloffMode.Custom;
			gunEndSource.maxDistance = 150f;
			gunEndSource.volume = soundVolume;
		}
		//
	...
	public void CountBullets()
	{
		...
		if (bulletType == BulletType.Rigidbody) {
			...
			if (ammunition == null){Debug.Log ("Gun " + transform.name + " ammunition gameobject has not been assigned");}
	...
	void Update () {
		...
		if(running && !gunLoopSource.isPlaying){gunLoopSource.Play();}
		if(!running && gunLoopSource.isPlaying)
		{gunLoopSource.Stop();gunEndSource.PlayOneShot(fireEndSound);}}
	}
...
public class GunEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		//
		GUILayout.Space(3f);
		gun.fireLoopSound = EditorGUILayout.ObjectField ("Fire Loop Sound", gun.fireLoopSound, typeof(AudioClip), true) as AudioClip;
		GUILayout.Space(3f);
		gun.fireEndSound = EditorGUILayout.ObjectField ("Fire End Sound", gun.fireEndSound, typeof(AudioClip), true) as AudioClip;
		GUILayout.Space(3f);
