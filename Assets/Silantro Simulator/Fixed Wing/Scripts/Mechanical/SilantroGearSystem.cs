...
public class SilantroGearSystem : MonoBehaviour {
	...
	public class WheelSystem
	{
		...
		public Quaternion initialWheelRotation;
		//
	...
	[HideInInspector]public SilantroActuator gearActuator;
	//TOGGLE GEAR
	...
	void BrakingSystem(WheelSystem wheel)
	{
		...
		if (ActualBrakeTorque > 10f) {
			if(wheel.collider != null)wheel.collider.brakeTorque = ActualBrakeTorque;
		}
	}
	...
	void Update()
	{
		if (allOk) {
			...
			foreach (WheelSystem system in wheelSystem) {
				...
				if (!system.collider.isGrounded && controller.transform.position.y > 5) {
	...
	public override void OnInspectorGUI()
	{
		...
		if (gear.gearStructType == SilantroGearSystem.GearStructType.Retractable) {
			...
			if (gear.systemType == SilantroGearSystem.SystemType.Custom) {
				EditorGUILayout.LabelField ("Put your custom gear system here");
			}
		}
		//
		GUILayout.Space (20f);
