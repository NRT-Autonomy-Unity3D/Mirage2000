...
public class SilantroHydraulicSystem : MonoBehaviour {
	...
	//------------------------SELECTIBLES
	public enum ControlType { Internal,Acutator}
	[HideInInspector] public ControlType controlType = ControlType.Internal;
	[HideInInspector] public SilantroActuator connectedActuator;
	...
	public void InitializeActuator()
	{
		...
		started = true;
		if(controlType == ControlType.Acutator) { openTime = 2f;closeTime = 2f; }
	}
	...
	void Update()
	{
		...
		if (open && currentState == CurrentState.Closed) {
			...
			if (!activated) {
				...
				//TEST
				if (controlType == ControlType.Acutator)
				{
					connectedActuator.DisengageActuator();
				}
			}
		}
		//OPENING MOVEMENT
			if (close && currentState == CurrentState.Open) {
				...
				if (!activated) {
					...
					if (controlType == ControlType.Acutator)
					{
						connectedActuator.EngageActuator();
					}
				}
			}
		}
...
public class HydraulicSystemEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space(3f);
		hydraulic.controlType = (SilantroHydraulicSystem.ControlType)EditorGUILayout.EnumPopup(" ", hydraulic.controlType);


		if (hydraulic.controlType == SilantroHydraulicSystem.ControlType.Acutator)
		{
			GUILayout.Space(5f);
			GUI.color = silantroColor;
			EditorGUILayout.HelpBox("Connected Actuator", MessageType.None);
			GUI.color = backgroundColor;
			GUILayout.Space(5f);
			hydraulic.connectedActuator = EditorGUILayout.ObjectField(" ", hydraulic.connectedActuator, typeof(SilantroActuator), true) as SilantroActuator;
		}
		if (hydraulic.controlType == SilantroHydraulicSystem.ControlType.Internal)
		{
			GUILayout.Space(5f);
			GUI.color = silantroColor;
