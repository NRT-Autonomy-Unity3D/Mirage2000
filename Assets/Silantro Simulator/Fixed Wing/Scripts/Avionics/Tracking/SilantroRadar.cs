...
public class SilantroRadar : MonoBehaviour {
	...
	[HideInInspector]public ObjectFilter objectFilter = ObjectFilter.All;
	//PROPERTIES
	...
	[HideInInspector]public LayerMask checkLayers = ~0;
	...
	private void Ping()
	{
		...
		airTargets = new List<FilteredObject>();groundTargets = new List<FilteredObject>();
		//FILTER
	...
	void Update()
	{
		if (isControllable && allOk && GameControl.instance.isTrainingStarts) {
			//SPIN CORE
	...
	private void OnGUI()
	{
		if (!isControllable || !GameControl.instance.isTrainingStarts || GameControl.instance.isWarningDisplaysInTrainingPage) {
			return;
		}
		//SET SKIN
		if (radarSkin != null) {
...
public class RadarEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space (5f);
		SerializedProperty layerMask = radarObject.FindProperty ("checkLayers");
		...
		GUILayout.Space (3f);
		radar.objectFilter = (SilantroRadar.ObjectFilter)EditorGUILayout.EnumPopup("Target Filter",radar.objectFilter);
		}
