...
public class SilantroLever : MonoBehaviour {
	...
	[HideInInspector]public ThrottleMode throttleMode = ThrottleMode.Deflection;
	...
	void Update () {
		...
		if (controller != null) {
			...
			if (leverType == LeverType.Throttle) {
				...
				//DEFLECTION
				if (throttleMode == ThrottleMode.Deflection) {
					lever.localRotation = InitialRotation;
					lever.Rotate (axisRotation, throttleAmount);
				}
				if (throttleMode == ThrottleMode.Sliding) {
					lever.localPosition = initialPosition;
					lever.localPosition += axisRotation * throttleAmount/100f;
				}
			}
			//
...
public class LeverEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if (lever.leverType == SilantroLever.LeverType.Throttle) {
			...
			GUILayout.Space (3f);
			lever.throttleMode = (SilantroLever.ThrottleMode)EditorGUILayout.EnumPopup("Mode",lever.throttleMode);
			...
			//
			GUILayout.Space (10f);
			lever.rotationAxis = (SilantroLever.RotationAxis)EditorGUILayout.EnumPopup ("Sliding Axis", lever.rotationAxis);
			GUILayout.Space (3f);
				lever.maximumDeflection = EditorGUILayout.FloatField ("Maximum Slide Distance", lever.maximumDeflection);
			}
		}
		//
