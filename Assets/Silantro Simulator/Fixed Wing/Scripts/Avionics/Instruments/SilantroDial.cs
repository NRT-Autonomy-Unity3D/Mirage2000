...
public class SilantroDial : MonoBehaviour {
	...
	//DIAL TYPE
	public enum DialType{Speed,Altitude,Fuel,VerticalSpeed,RPM,BankLevel,Compass,AngleOfAttack,ArtificialHorizon,Accelerometer,FuelFlow,NozzlePosition,Mach,AirTemperature,Clock,FlapAngle}
	...
	public void Calculations()
	{
		...
		if (dialType == DialType.ArtificialHorizon) {
			...
			var roll = Aircraft.eulerAngles.z;
			if (negativeYawRotation) {yaw *= -1f;}
			if (negativePitchRotation) {pitch *= -1f;}
			if(negativeRollRotation){roll *= -1f;}
			//
...
public class DialEditor: Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		if (dial.dialType != SilantroDial.DialType.ArtificialHorizon && dial.dialType != SilantroDial.DialType.Clock) {
			...
			GUILayout.Space (5f);
			dial.rollRotationAxis = (SilantroDial.RollRotationAxis)EditorGUILayout.EnumPopup ("Roll Rotation Axis", dial.rollRotationAxis);
			GUILayout.Space (2f);
			GUILayout.Space (2f);
			dial.negativeRollRotation = EditorGUILayout.Toggle ("Invert Roll Rotation", dial.negativeRollRotation);
			//
			GUILayout.Space (15f);
			GUI.color = silantroColor;
			EditorGUILayout.HelpBox ("Connections", MessageType.None);
