...
public class SilantroCamera : MonoBehaviour
{
    ...
    void InteriorSystem()
    {
        ...
        {
            if (Input.touchCount == 1 && interiorcamera != null && Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                ...
                interiorcamera.transform.localRotation = localRotation;
            }
        }
    }
    ...
    void FreeSystem()
    {
        //FREE CAMERA SYSTEM
        if (Input.GetMouseButton(0))
        {//Only rotate if left mouse button is pressed  down
