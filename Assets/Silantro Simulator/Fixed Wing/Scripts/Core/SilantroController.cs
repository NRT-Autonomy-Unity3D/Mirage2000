...
public class SilantroController : MonoBehaviour {
	...
	//
	public enum InputType{Default,Rewired}
	...
	[HideInInspector]public SilantroActuator[] actuators;
	...
	//CUSTOM INPUT - REWIRED
	public int rewiredPlayerId = 0;
	private Player rewiredPlayerInput; // The Rewired Player
	...
	{
		InitializeAircraft ();
		if (PlayerPrefs.HasKey("FlightInput"))
		{
			inputKey = PlayerPrefs.GetString("FlightInput");
			SetInput(inputKey);
		}
		else
		{
			string input = inputType.ToString().ToLower();
			PlayerPrefs.SetString("FlightInput", input);
			SetInput(input);
		}
	}
	...
	{
		if (input == InputType.Rewired.ToString().ToLower())
		{
			// Get the Rewired Player object for this player and keep it for the duration of the character's lifetime
			rewiredPlayerInput = ReInput.players.GetPlayer(rewiredPlayerId);
			GameControl.instance.SetInput(1);
		}
		else
		{
			GameControl.instance.SetInput(0);
		}
	}
	...
	void CollectInputs()
	{
		...
		//2. 
		else if(inputType == InputType.Rewired) 
		{
			//PUT CUSTOM CONTROL CODE HERE
			//COLLECT INPUT VARIABLES
			//1. MAIN
			pitchInput = rewiredPlayerInput.GetAxis("Pitch");
			rollInput = rewiredPlayerInput.GetAxis("Roll");
			yawInput = rewiredPlayerInput.GetAxis("Rudder");
			//2. ENGINES
			float baseInput = (rewiredPlayerInput.GetAxis("Throttle"));
			throttleInput = (baseInput + 1) / 2;

			//2.1 START AND STOP
			if (rewiredPlayerInput.GetButtonDown("Start Engine"))
			{
				TurnOnEngines();
			}
			if (rewiredPlayerInput.GetButtonDown("Stop Engine"))
			{
				TurnOffEngines();
			}
			//3. BRAKES
			...
			{
				//1. INCREMENTAL BRAKE
				if (rewiredPlayerInput.GetButton("Brake Lever"))
				{
					brakeControl = Mathf.Lerp(brakeControl, 1f, 0.1f);
				}
				else
				...
				//2. PARKING BRAKE
				if (rewiredPlayerInput.GetButtonDown("Parking Brake"))
				{
					gearHelper.ToggleBrake();
				}
			}
			//4. SET TCS - KeyCode.T 
			if (rewiredPlayerInput.GetButtonDown("Set TCS"))
			{
				foreach (SilantroAerofoil wing in wings)
				{
					//wing.ToggleTCS ();
			...
			//5. FLAP INCREASE
			if (rewiredPlayerInput.GetButtonDown("Extend Flap"))
			{
				foreach (SilantroAerofoil wing in wings)
				{
					wing.LowerFlap();
			...
			//5.1 FLAP DECREASE
			if (rewiredPlayerInput.GetButtonDown("Retract Flap"))
			{
				foreach (SilantroAerofoil wing in wings)
				{
					wing.RaiseFlap();
			...
			//6. SLAT DECREASE
			if (rewiredPlayerInput.GetButtonDown("Actuate Slat"))
			{
				foreach (SilantroAerofoil wing in wings)
				{
					wing.ActuateSlat();
			...
			//7. SPOILER DECREASE
			if (rewiredPlayerInput.GetButtonDown("Spoiler"))
			{
				foreach (SilantroAerofoil wing in wings)
				{
					wing.ActuateSpoilers();
			...
			//8. CHANGE CAMERA
			if (rewiredPlayerInput.GetButtonDown("Toggle Camera"))
			{
				if (view != null)
				{
					view.ToggleCamera();
			...
			//9. TOGGLE GEAR
			if (rewiredPlayerInput.GetButtonDown("Actuate Gear"))
			{
				if (gearHelper != null)
				{
					gearHelper.ToggleGear();
			...
			//10. TOGGLE SPEEDBRAKES
			if (rewiredPlayerInput.GetButtonDown("Speed Brake"))
			{
				if (speedBrakes != null)
				{
					speedBrakes.ToggleSpeedBrake();
			...
			//11. TOGGLE LIGHT
			if (rewiredPlayerInput.GetButtonDown("LightSwitch"))
			{
				if (lightControl != null)
				{
					lightControl.ToggleLight();
			...
			//12. TOGGLE AFTERBURNER
			if (rewiredPlayerInput.GetButtonDown("Afterburner"))
			{
				//TURBO JET
				if (engineType == EngineType.TurboJet)
				{
			...
			//13. TOGGLE REVERSE THRUST
			if (rewiredPlayerInput.GetButtonDown("Reverse Thrust"))
			{
				if (engineType == EngineType.TurboFan)
				{
					foreach (SilantroTurboFan engine in turboFans)
			...
			//14. SWING WING - KeyCode.J
			if (rewiredPlayerInput.GetButtonDown("Swing Wing"))
			{
				if (wingActuator != null)
				{
					if (leftWing.flapDeflection < 1)
			...
			//15. RELOAD SCENE - KeyCode.R
			if (rewiredPlayerInput.GetButtonDown("Reload Scene"))
			{
				ResetScene();
			}
			//16. EXIT AIRCRAFT - KeyCode.F
			if (controlType == ControlType.Internal && !temp && pilotOnboard && rewiredPlayerInput.GetButtonDown("Exit Aircraft"))
			{
				//EXIT CONDITION
				if (isGrounded && coreSystem.currentSpeed < 10)
				{
			...
			//17. GO TO VTOL
			if (rewiredPlayerInput.GetButtonDown("Transition To VTOL"))
			{
				if (aircraftType == AircraftType.VTOL)
				{
					VTOLTransition();
			...
			//18. GO TO STOL
			if (rewiredPlayerInput.GetButtonDown("Transition To STOL"))
			{
				if (aircraftType == AircraftType.VTOL)
				{
					STOLTransition();
			...
			//19. GO TO Normal
			if (rewiredPlayerInput.GetButtonDown("Transition To Normal"))
			{
				if (aircraftType == AircraftType.VTOL)
				{
					NormalTransition();
			...
			{
				//20 SHOOT GUN
				if (rewiredPlayerInput.GetButton("Fire"))
				{
					//FIRE GUN IF SELECTED
					if (Armaments.currentWeapon == "Gun")
					{
				...
				//
				//21. ROCKETS - KeyCode.Alpha1 (Fire Rocket)
				if (rewiredPlayerInput.GetButton("Weapon Release") && rewiredPlayerInput.GetButton("Fire Rocket"))
				{
					Armaments.FireRocket();
				}
				//
				//22. MISSILES - KeyCode.Alpha2 (Fire Missile)
				if (rewiredPlayerInput.GetButton("Weapon Release") && rewiredPlayerInput.GetButton("Fire Missile"))
				{
					Armaments.FireMissile();
				}
				//
				//23. BOMBS - KeyCode.Alpha3 (Fire Bomb)
				if (rewiredPlayerInput.GetButton("Weapon Release") && rewiredPlayerInput.GetButton("Fire Bomb"))
				{
					Armaments.DropBomb();
				}
				//24. FIRE SELECTED WEAPON
				if (rewiredPlayerInput.GetButtonDown("Fire"))
				{
					//MISSILE
					if (Armaments.currentWeapon == "Missile")
					{
				...
				//25. SELECT WEAPON
				if (rewiredPlayerInput.GetButtonDown("Weapon Select"))
				{
					Armaments.ChangeWeapon();
				}
			}
			...
			{
				//22. SELECT UPPER TARGET
				if (rewiredPlayerInput.GetButtonDown("Target Up"))
				{
					radarCore.SelectedUpperTarget();
				}
				//23. SELECT LOWER TARGET
				if (rewiredPlayerInput.GetButtonDown("Target Down"))
				{
					radarCore.SelectLowerTarget();
				}
				//24. LOCK SELECTED TARGET
				if (rewiredPlayerInput.GetButtonDown("Target Lock"))
				{
					radarCore.LockSelectedTarget();
				}
				//25. RELEASE LOCK - KeyCode.Backspace
				if (rewiredPlayerInput.GetButtonDown("Release Lock"))
				{
					radarCore.ReleaseLockedTarget();
				}
			}
	...
	void CollectData()
	{
		...
		if(actuators != null)
		{
			...
			{
				if(actuator.generatesDrag){additionalDrag += actuator.currentDragFactor;}
			}
		}
		//COLLECT DRAG
		if (additionalDrag <= 0) {
	...
	void Update()
	{
		if (allOk && isControllable && GameControl.instance.isTrainingStarts) {
			//COLLECT DATA
			CollectData ();
			//COLLECT INPUT
			CollectInputs ();
	...
	public void InputSelection (int id)
	{
		if (id == 0)
		{
		...
		else if (id == 1)
		{
			...
			inputType = InputType.Rewired;
		}
		PlayerPrefs.SetString("FlightInput", inputType.ToString().ToLower());
	}
