...
public class SilantroCore : MonoBehaviour {
	...
	[HideInInspector]public float pitchRate, rollRate, yawRate;
	...
	void Update()
	{
		...
		if (stallAngles.Count > 0) {
			...
			//CHECK
			if (Mathf.Abs (stallThreshold) >= (Mathf.Abs (minimumStallAngle) - stallLimitFactor) && currentSpeed > speedThreshold) {stalling = true;} else {stalling = false;}
			//PLAY WARNING
			if (warner != null) {
	...
	void CalculateData(float altitude)
	{
		...
		XbankAngle = aircraft.transform.eulerAngles.x;if (XbankAngle > 180) {	XbankAngle -= 360;}
		YbankAngle =  aircraft.transform.eulerAngles.y;if (YbankAngle > 180) {YbankAngle -= 360;}
		ZbankAngle = aircraft.transform.eulerAngles.z;if ( ZbankAngle > 180.0f ){ZbankAngle = -(360.0f - ZbankAngle);}
		...
		yawRate = localAngularVelocity.y * Mathf.Rad2Deg;
		...
		gForce = turnForce / -9.81f; gForce += transform.up.y * (Physics.gravity.y / -9.81f);
